...
O WebFront (interface web em canvas) é onde a esfera é construída na prática: captura interativa, tags de natureza/caminho e sugestão via IA.
Este módulo faz o lado Python do "contrato" combinado com script.js (botão "Exportar sessão (JSON)"): lê esse .json e reconstrói um objeto Grafo idêntico ao que Main.py usaria, pra então calcular as métricas formais do método (raio, densidade, eficiência, produtividade) e, se quiser, gerar a visualização interativa via pyvis.
...
def Carregar_De_Json(caminho: str) -> Grafo:
    ...
    for no_meio in grafo.Nodes:
        ...
        # Como o JS agora exporta Teto -> Meio -> Piso,
        # as origens do nó do meio (vizinhos_inv) são o Teto (gis)
        # e os destinos (vizinhos) são o Piso (eis)
        gis = [v.valor for v in no_meio.vizinhos_inv]
        eis = [v.valor for v in no_meio.vizinhos]
